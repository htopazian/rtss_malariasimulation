...
- Code up a new 'hybrid' strategy: EPI programmatic roll-out of RTS,S doses 1-3 with seasonal boosters. Compare this hybrid strategy to standard EPI and seasonal vaccination strategies by measuring intervention efficacy across seasonal settings.
...
- Determine the coverage achieved (% vaccinated) under EPI vs. mass vaccination settings.  

- Measure the relative cost-effectiveness of investing in ITNs vs. RTS,S. 
...
**Figure 2.** Distribution of severe cases by age and transmission setting. Age represents the upper end of a 1 year age-group. E.g. age "5" represents children ages 4-5. Results are drawn from a baseline model which includes no RTS,S or SMC intervention.
...
# plot
ggplot(output) + 
  geom_line(aes(x=age_high, y=s_cases, color=as.factor(pfpr)), size=1) + 
...
**Table 1.** Description of RTS,S implementation strategies.
...
dose13 <- c("6, 7.5, and 9 months of age", "3 months before the high transmission season, among those 5-17 months of age", "6, 7.5, and 9 months of age")
...
**Figure 3a.** The number of cases per year under each intervention strategy, stratified by season and PfPR<sub>2-10</sub> transmission setting. 
...
**Figure 3b.** The number of cases per year under each intervention strategy, stratified by season and PfPR<sub>2-10</sub> transmission setting. 
...
**Figure 4.** Averted - The following plot compares clinical cases, severe cases, and deaths over the whole 15 simulation period, stratified by intervention.
...
**Table 2.** Number of clinical cases as the minimum wait time varying between receiving the 3rd dose and the first booster dose under the hybrid strategy. The outcome is the number of cases among children 0-5 years under a PfPR<sub>2-10</sub> = 0.55 transmission setting, stratified by and seasonality. 
