...
#   ggplot(data=data, aes(x=year, y=cases, color=intervention)) + 
#     geom_line() + 
#     scale_y_continuous(labels = comma) + 
#     scale_x_continuous(breaks = seq(1,15,1)) + 
#     facet_wrap(pfpr ~ ., nrow = 1) +
#     labs(title = title, x='Year', x='Cases', color=expression(italic(PfPR[2-10]~(symbol("\045"))))) +
#     theme_classic()
# }
# 
# A <- plotcompare(output2 %>% filter(seasonality=='high_seas'), 'highly seasonal')
# B <- plotcompare(output2 %>% filter(seasonality=='low_seas'), 'seasonal')
# 
...
ggplot(data=output2, aes(x=year, y=cases, group=interaction(pfpr, intervention), color=intervention)) + 
  ...
  facet_grid(. ~ seasonality, labeller = labeller(seasonality = seas.labs)) + 
  labs(x='Year', y='Cases', color='intervention') +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 90, hjust=1, vjust=0.5)) + 
  annotate("text", x = 18, y = 22000, label = 'PfPR 10') + 
  annotate("text", x = 18, y = 70000, label = 'PfPR 25') + 
  annotate("text", x = 18, y = 110000, label = 'PfPR 35') + 
  annotate("text", x = 18, y = 220000, label = 'PfPR 55') 

```

```{r compare2, message=FALSE, warning=FALSE, fig.width=8, fig.height=12}
output3 <- output2 %>% 
...
compareplot <- function(data, var, title){ggplot(data=output3, aes(x=factor(pfpr), y=cases, group=intervention, fill=intervention)) + 
...
compareplot(output3, output3$cases, 'Clinical cases (per 100,000 children 0-5 years)') / 
  compareplot(output3, output3$severe, 'Severe cases (per 100,000 children 0-5 years)') / 
  compareplot(output3, output3$deaths, 'Deaths (per 100,000 children 0-5 years)') + plot_layout(guides = "collect") 

```

<br>

# 2. Explore minimum wait time between 3rd and 4th dose (booster)
...
```{r wait time, message=FALSE, warning=FALSE, fig.width=8, fig.height=6}
output <- readRDS('C:/Users/htopazia/OneDrive - Imperial College London/Github/rtss_malariasimulation/rds/rtss_hybrid_all.rds') 
...
```{r wait time3, message=FALSE, warning=FALSE, fig.width=10, fig.height=6}
...
# 3. Advantages of adding a fifth dose


```{r fifth dose, message=FALSE, warning=FALSE, fig.width=8, fig.height=6}
output <- readRDS('C:/Users/htopazia/OneDrive - Imperial College London/Github/rtss_malariasimulation/rds/rtss_hybrid_all.rds') 
...
none <- output %>% filter(grepl('none', file)) %>% 
  ...
  summarize(cases_base = sum(cases05_p, na.rm=T),
            severe_base = sum(severe05_p, na.rm=T),
            deaths_base = sum(deaths05_p, na.rm=T)) %>% ungroup() 
  
# plot
output2 <- output %>% 
  # limit dataset to SV and hybrid scenarios and rename
  ...
  mutate(case_avert = cases_base - cases,
         severe_avert = severe_base - severe,
         death_avert = deaths_base - deaths)
...
ggplot(data=output2, aes(x=year, y=case_avert, group=interaction(pfpr, intervention), color=intervention)) + 
  ...
  scale_x_continuous(breaks = seq(1,15,1), limits = c(0,20)) + 
  facet_grid(. ~ seasonality, labeller = labeller(seasonality = seas.labs)) + 
  labs(x='Year', y='Cases averted', color='intervention') +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 90, hjust=1, vjust=0.5)) + 
  annotate("text", x = 18, y = 5000, label = 'PfPR 10') +
  annotate("text", x = 18, y = 19000, label = 'PfPR 25') +
  annotate("text", x = 18, y = 25000, label = 'PfPR 35') +
  annotate("text", x = 18, y = 37000, label = 'PfPR 55')
```
...
output3 <- output2 %>% 
  ...
  summarize(case_avert = sum(case_avert, na.rm=T),
            severe_avert = sum(severe_avert, na.rm=T),
            death_avert = sum(death_avert, na.rm=T)) %>% ungroup() 
  
ggplot(data=output3, aes(x=factor(pfpr), y=case_avert, group=intervention, fill=intervention)) + 
  geom_col(position = 'dodge') + 
  facet_grid(seasonality  ~ ., labeller = labeller(seasonality = seas.labs)) + 
  labs(x=expression(paste(italic(Pf),"PR"[2-10])), y='Cases averted', fill='intervention') +
  theme_classic() 
...
# 4. Coverage achieved under EPI vs. mass vaccination settings

```{r coverage, message=FALSE, warning=FALSE, fig.width=7, fig.height=10}
output <- readRDS('C:/Users/htopazia/OneDrive - Imperial College London/Github/rtss_malariasimulation/rds/rtss_hybrid_all.rds') 
...
output2 <- output %>%
  ...
  mutate(dose1 = sum(dose1, na.rm=T) / n_0_1825,
         dose2 = sum(dose2, na.rm=T) / n_0_1825,
         dose3 = sum(dose3, na.rm=T) / n_0_1825,
         dose4 = sum(dose4, na.rm=T) / n_0_1825,
         dose5 = sum(dose5, na.rm=T) / n_0_1825) %>%
...
# 5. Cost-effectiveness of ITNs vs. RTS,S
