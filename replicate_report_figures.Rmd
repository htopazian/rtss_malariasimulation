...
match <- PfPR %>%
  ...
  rename(eir=init_EIR) %>% 
  mutate(eir = as.numeric(round(eir,1)), seasonality=case_when(season=='high seasonality'~'highly_seasonal', TRUE~'seasonal')) %>% ungroup() %>% select(-season)

match

```

EIR values are 1.1, 4.7, 10.1, and 38.2 in highly seasonal settings.
EIR values are 1.2, 4.0, 8.7, and 32.6 in seasonal settings.
...
baselineB <- readRDS("C:/Users/htopazia/OneDrive - Imperial College London/Github/rtss_malariasimulation/rds/rtss_smc_all.rds") %>% 
  ...
  left_join(match %>% select(pfpr, eir, seasonality))
