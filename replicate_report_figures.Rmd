...
baselineA <- readRDS('C:/Users/htopazia/OneDrive - Imperial College London/Github/Hayley_report/Part_1/combined_outputs_raw.RDS') %>% 
  ...
  group_by(run, pfpr, seasonality, year) %>%
  summarise(cases = sum(cases,na.rm=T),
            deaths = sum(deaths, na.rm=T),
            severe = sum(severe, na.rm=T)) %>%
  filter(pfpr %in% c(0.1,0.25,0.35,0.55)) %>%
  ungroup() %>% mutate(model='MalariaLaunchR')
...
baselineA <- oldmodelprocess('C:/Users/htopazia/OneDrive - Imperial College London/Github/Hayley_report/Part_1/combined_outputs_raw.RDS') %>% 
  rbind(oldmodelprocess('C:/Users/htopazia/OneDrive - Imperial College London/Github/Hayley_report/Part_2/combined_outputs_smc.RDS'))
