...
p2 <- p2 %>% as_tibble() %>% dplyr::select(x,y,PANEL) %>% rename(init_EIR=x, pred=y, season=PANEL) %>%
...
match <- PfPR %>%
  ...
  rename(eir=init_EIR) %>% 
  mutate(eir = as.numeric(signif(eir,3)), seasonality=case_when(season=='high seasonality'~'highly_seasonal', TRUE~'seasonal')) %>% ungroup() %>% dplyr::select(-season)
...
```{r Figure1, message=FALSE, warning=FALSE, fig.width=12, fig.height=4}
...
fig1dat <- output %>% 
  left_join(match %>% dplyr::select(pfpr, eir)) %>% 
...
# plot ----------------------------------------------------------------------
ca + Fig1a + plot_layout(guides = "collect") + plot_annotation(title = 'A - Clinical cases averted per 100,000 population', theme = theme(plot.title = element_text(size = 18)))

ca_fvp + Fig1b + plot_layout(guides = "collect") + plot_annotation(title = 'B - Clinical cases averted (per 100,000 fully vaccinated children', theme = theme(plot.title = element_text(size = 18)))

da + Fig1c + plot_layout(guides = "collect") + plot_annotation(title = 'C - Deaths averted per 100,000 population', theme = theme(plot.title = element_text(size = 18)))

da_fvp + Fig1d + plot_layout(guides = "collect") + plot_annotation(title = 'D - Deaths averted (per 100,000 fully vaccinated children)', theme = theme(plot.title = element_text(size = 18)))
...
```{r Figure2, message=FALSE, warning=FALSE, fig.width=12, fig.height=4}
# MalariaLaunchR ------------------------------------------------------------
...
figure_2_pd <- figure_2_pd %>% 
  mutate(run = case_when(run=="epi_smc" ~ "EPI + SMC", 
...
figure_2_pd$run <- factor(figure_2_pd$run, levels=c("SMC alone", "EPI + SMC", "SV 4-dose + SMC", "SV 4-dose - updated booster + SMC", "SV 4-dose synergy + SMC", "SV 5-dose + SMC", "SV 5-dose - updated booster + SMC", "SV 5-dose synergy + SMC"))
...
# plot ----------------------------------------------------------------------
ca + Fig2a + plot_layout(guides = "collect") + plot_annotation(title = 'A - Clinical cases averted per 100,000 population', theme = theme(plot.title = element_text(size = 18)))

da + Fig2b + plot_layout(guides = "collect") + plot_annotation(title = 'B - Deaths averted per 100,000 population', theme = theme(plot.title = element_text(size = 18)))
...
C <- A %>% full_join(B) %>%
  ...
  mutate(intervention = case_when(run=="epi" ~ "EPI",
      ...
      run=="srtss_5_dose_param_mod_smc" ~ "SV 5-dose synergy + SMC",
      run=="smc" ~ "SMC alone")) %>%
   dplyr::select(intervention, pfpr, seasonality, cases, deaths, cases_averted, deaths_averted, cases_averted_per_100000_fvp, number_fvp) 
...
For `malariasimulation` RTS,S dose #3 efficacy starts at month 4 for seasonal settings and month 6 for highly seasonal settings. SMC rounds are spaced so that it covers the high transmission season. RTS,S is shown in blue and SMC in red below. 
...
Now we will directly compare model outputs from `malariasimulation` to `MalariaLaunchR`. 

```{r baseline, message=FALSE, warning=FALSE, fig.width=7, fig.height=10}
#-New facet label names for seas variables--

eir <- c(2.19, 9.47, 20.5, 75, 1.81, 7.74, 15.8, 61.3)
pfpr <- rep(c(.10, .25, .35, .55), 2)
season <- c(rep('highly_seasonal',4), rep('seasonal',4))

match <- data.frame(eir, pfpr, season) 
...
baselineB <- readRDS("C:/Users/htopazia/OneDrive - Imperial College London/Github/rtss_malariasimulation/rds/rtss_smc_all.rds") %>% 
  ...
  summarize(cases = sum(cases, na.rm=T),
            severe = sum(severe_p, na.rm=T),
            deaths = sum(deaths_p, na.rm=T),
            model='malariasimulation') %>% ungroup() %>%
  left_join(match %>% dplyr::select(pfpr, eir))
...
```{r immunity comparison, message=FALSE, warning=FALSE, fig.width=12, fig.height=8}
immunity <- readRDS("C:/Users/htopazia/OneDrive - Imperial College London/Github/rtss_malariasimulation/rds/rtss_smc_all.rds") %>% 
  mutate(year=ceiling(timestep/365)) %>%
  group_by(year, eir) %>%
  mutate(ica_mean = mean(ica_mean, na.rm=T), 
         icm_mean = mean(icm_mean, na.rm=T),
         iva_mean = mean(iva_mean, na.rm=T), 
         ivm_mean = mean(ivm_mean, na.rm=T)) %>%
  rename(seasonality=season) %>%
  left_join(match %>% dplyr::select(pfpr, eir)) %>% filter(intervention == "none")

seas.labs <- c("Highly Seasonal", "Seasonal")
names(seas.labs) <- c("highly_seasonal", "seasonal")

immunityplot <- function(data, var, title){
ggplot(data=data, aes(x=year, y=var, color=factor(pfpr * 100))) + 
      geom_line() + 
      labs(x = expression(paste(italic(Pf),"PR"[2-10])),
      y = " ",
      parse=TRUE) +
      ...
      scale_x_continuous(breaks = seq(1,15,1)) +
      facet_wrap(seasonality  ~ ., labeller = labeller(seasonality = seas.labs)) +
      labs(title = title, x='year', color=expression(italic(PfPR[2-10]~(symbol("\045"))))) +
      theme_classic()
}


A <- immunityplot(immunity, immunity$ica_mean, 'ica_mean')
B <- immunityplot(immunity, immunity$icm_mean, 'icm_mean')
C <- immunityplot(immunity, immunity$iva_mean, 'iva_mean')
D <- immunityplot(immunity, immunity$ivm_mean, 'ivm_mean')
...
Hayley is calculating this here: https://github.com/htopazian/seasonal_use_case/blob/main/Part_1/4_post_processing.R#L125-L149. I'm not sure if the code is correctly adjusting back to dose 3 for scenarios where children receive 1 booster (EPI and SV 4-dose). The number of children receiving dose 3 should be the same under all booster scenarios. 
...
vaccineA <- readRDS('C:/Users/htopazia/OneDrive - Imperial College London/Github/Hayley_report/Part_1/combined_outputs_raw.RDS') %>% 
  ...
  dplyr::mutate(
   ...
   dplyr::select(run, pfpr, seasonality, draw, coverage, year, num_vacc_doses,  num_trt) %>%
  filter(year > 0) %>%
  filter(coverage %in% c(0.8,0)) %>%
  filter(pfpr %in% c(0.1,0.25,0.35,0.55)) %>%
...
`malariaEquilibrium` uses the average age to calculate an exponential demography. This does not match up with the `mlgts::flat_demog` file Hayley used. I am including a code snippet from [malariaEquilibrium](https://github.com/mrc-ide/malariaEquilibrium/blob/b331dab66cac15a8e0ec91171265877f6de92b09/R/main.R#L58-L90), but it does not appear to be working ... 
...
flat_demog %>% rename(year = V1, age_lower = V2, age_upper = V3, inflow = V4, outflow = V5)
...
# Things to check
  ...
  * MalariaLaunchR uses a constant population size and demography based on the life table for Butajira, Ethiopia, with an average life expectancy at birth of 46.6 years. Test malariasimulation with the flat_demog.txt file when the model can handle demography inputs. 
  ...
  * SMC rounds - the old model runs 3 rounds, but it was supposed to be 4, Hayley will send an updated rds file.
  
  
```{r severe check, fig.width=10, fig.height=4}

baselineA <- readRDS('C:/Users/htopazia/OneDrive - Imperial College London/Github/Hayley_report/Part_1/combined_outputs_raw.RDS')
...
baselineA <- baselineA %>% 
  ...
  filter(year > 0) %>%
  filter(coverage %in% c(0.8,0)) %>%
  filter(age_lower %in% c(seq(0,4,1), seq(25,49,1))) %>%
  mutate(under5 = ifelse(age_lower >= 0 & age_lower <5, 1, 0)) %>% 
  group_by(run, pfpr, seasonality, coverage, draw, year, under5) %>%
  summarise(across(cases:deaths, sum)) %>%
  ungroup() %>% group_by(run, pfpr, seasonality, coverage, draw, year, under5) %>%
  summarise(severe = sum(severe, na.rm=T)) %>%
  filter(pfpr %in% c(0.1,0.25,0.35,0.55)) %>%
  mutate(s05 = ifelse(under5==1, severe, NA),
         s2550 = ifelse(under5==0, severe, NA)) %>% dplyr::select(-under5) %>%
  summarise(s05 = sum(s05, na.rm=T),
            s2550 = sum(s2550, na.rm=T)) %>% mutate(model='MalariaLaunchR') %>% ungroup() 
...
baselineB <- readRDS('C:/Users/htopazia/OneDrive - Imperial College London/Github/rtss_malariasimulation/rds/rtss_smc_all.rds') %>%
  ...
  rename(seasonality=season) %>%
  group_by(intervention, eir, seasonality, year) %>%
  summarize(s05 = sum(p_inc_severe_0_1825, na.rm=T),
            s2550 = sum(p_inc_severe_9125_18250, na.rm=T)) %>%
  left_join(match %>% dplyr::select(pfpr, eir)) %>% 
  mutate(pfpr = ifelse(is.na(pfpr), .55, pfpr), model = 'malariasimulation')

dat <- baselineA %>% filter(run == "baseline") %>% full_join(baselineB %>% filter(intervention == 'none'))

seas.labs <- c("Highly Seasonal", "Seasonal")
names(seas.labs) <- c("highly_seasonal", "seasonal")

A <- ggplot() + 
  #geom_line(data=dat, aes(x=year, y = s05, color=factor(pfpr * 100))) + 
  geom_line(data=dat, aes(x=year, y = s2550, color=factor(pfpr * 100), lty=model)) + 
  labs(x = expression(paste(italic(Pf),"PR"[2-10])), 
       y = " ",
       parse=TRUE) + 
  scale_y_continuous(labels = comma) + 
  scale_x_continuous(breaks = seq(1,15,1)) + 
  facet_wrap(seasonality  ~ ., labeller = labeller(seasonality = seas.labs)) +
  labs(x='year', title='25-50 years', color=expression(italic(PfPR[2-10]~(symbol("\045"))))) +
  theme_classic()

B <- ggplot() + 
  geom_line(data=dat, aes(x=year, y = s05, color=factor(pfpr * 100), lty=model)) + 
  #geom_line(data=dat, aes(x=year, y = s2550, color=factor(pfpr * 100))) + 
  labs(x = expression(paste(italic(Pf),"PR"[2-10])), 
       y = " ",
       parse=TRUE) + 
  scale_y_continuous(labels = comma) + 
  scale_x_continuous(breaks = seq(1,15,1)) + 
  facet_wrap(seasonality  ~ ., labeller = labeller(seasonality = seas.labs)) +
  labs(x='year', title='0-5 years', color=expression(italic(PfPR[2-10]~(symbol("\045"))))) +
  theme_classic()
